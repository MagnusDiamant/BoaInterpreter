...
instance Monad RWSP where
  return a = RWSP (\_ x -> (a, mempty, x))
  m >>= f = RWSP (\z x -> let (v1, w1, s1) = runRWSP m z x 
                          in let (v2, w2, s2) = runRWSP (f v1) z s1
                              in (v2, w1 <> w2, s2)) 
...
instance Monad RWSE where
  return a = RWSE (\_ x -> Right (a, mempty, x))
  m >>= f = RWSE (\z x -> case runRWSE m z x of 
                            Left (e1) -> Left e1
                            Right (v1, w1, s1) -> case runRWSE (f v1) z s1 of 
                                                    Left (e2) -> Left e2
                                                    Right (v2, w2, s2) -> Right (v2, (w1 <> w2), s2))
